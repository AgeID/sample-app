...
class AgeIdController extends Controller
{
    ...
    public function modalCallback(Request $request)
    {
        ...
        if ($request->has('code')) {
            $ageIdResponse = $ageIdApiWrapper->callWithCode($request->get('code'), $request->ip());
        } else
    ...
    public function redirect()
    {
        $payloadHelper = new PayloadHelper();
        $payloadData = $payloadHelper->generateForRedirectHandshake();
        $url = \Config::get('ageId.baseURL') . '/sso/v1/handshake?pilot=redirect&client_id=' . $payloadHelper->clientId . '&payload=' . $payloadData;
